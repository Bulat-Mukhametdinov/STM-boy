...
static uint32_t usb_class_data[(sizeof(USBD_CDC_HandleTypeDef) + 3u) / 4u];

void *USBD_static_malloc(uint32_t size)
{
    if (size > sizeof(usb_class_data)) {
        return NULL;
    }
    USBD_memset(usb_class_data, 0, sizeof(usb_class_data));
    return usb_class_data;
}
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *hpcd)
{
    ...
    gpio.Pin = USB_FS_DM_PIN | USB_FS_DP_PIN;
    gpio.Mode = GPIO_MODE_AF_PP;
    gpio.Pull = GPIO_NOPULL;
    gpio.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    gpio.Alternate = USB_FS_AF;
    HAL_GPIO_Init(USB_FS_DM_GPIO_PORT, &gpio);
...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef *hpcd)
{
    if (hpcd->Instance == USB_OTG_FS) {
        ...
        HAL_GPIO_DeInit(USB_FS_DM_GPIO_PORT, USB_FS_DM_PIN | USB_FS_DP_PIN);
